...
import { Card, Col, Row, Button, Divider, message } from "antd";
...
const SubscriptionGroups = ({ shopOrigin }) => {
  ...
  const handleClickSubmit = async () => {
    ...
    await axios
      .post("http://localhost:9011/api/subscriptions/create", {
        shop: shopOrigin,
        accessToken: shopDetails.accessToken,
        name: groupName,
        merchantCode: groupName.replaceAll(" ", "-"),
        plans: [...subscriptions],
        productVariants: [
          "gid://shopify/ProductVariant/40050466947249",
          "gid://shopify/ProductVariant/40050466980017",
          "gid://shopify/ProductVariant/40050467012785",
        ],
      })
      .then((response) => {
  ...
  const handleClickViewProducts = () => { };
  const handleClickSelectProducts = () => {
  ...
  return (
    <DashboardLayout>
      ...
      <ResourcePicker resourceType="Product" open={selectProducts} allowMultiple={true} showVariants={true} onCancel={() => setSelectProducts(false)} onSelection={(e) => {

        let prodIds = e.selection.map(products => products.id);
        let arr = []
      ...
      <section>
        <Row gutter={[10, 10]}>
          <Col lg={6} sm={24} md={24}>
            ...
            <p style={styles.para}>
              The name of your subscription group is just for you to see, your
              customers will not see this name. Your products can only belong to
              one subscription group at a time.
            </p>
          </Col>
          <Col lg={2} />
          <Col lg={16} md={24} sm={24}>
            <Card style={styles.card}>
              ...
              <Button
                ...
                onClick={handleClickSelectProducts}
              >
                Select Products
              </Button>
              <PolarisButton plain onClick={handleClickViewProducts}>
        ...
        <Row gutter={[10, 10]}>
          <Col lg={6} sm={24} md={24}>
            ...
            <p style={styles.para}>
              ...
              subscribe.
              <br /> You can specify a day of the week or month where orders are
              generated.Advanced options allow for frequencies to match your
              fulfillment cycle using cutoff days.
            </p>
          </Col>
          <Col lg={2} />
          <Col lg={16} md={24} sm={24}>
